...
resource "aws_security_group" "my_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-my-webservers"
  }
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.my_webservers.id]
...
resource "aws_lb" "my-load-balancer" {
  name               = "${var.ENVIRONMENT}-my-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.my_webservers_alb.id]
